...
var packageMethods = {
  ...
  _fetchCode: function(path, cb) {
    ...
    globPath = process.platform === "win32" ? path.pathForGlob().replace(/\\/g, '/') : path.pathForGlob();
...
function expandPath(path, cb) {
  ...
  var globPath = process.platform === "win32" ? path.pathForGlob().replace(/\\/g, '/') : path.pathForGlob();
